...
public class DocumentationBuilder {
  ...
  public DocumentationBuilder name(String groupName) {
    this.groupName = BuilderDefaults.defaultIfAbsent(
        groupName,
        this.groupName);
    return this;
  }
  ...
  public DocumentationBuilder resourceListing(ResourceListing resourceListing) {
    this.resourceListing = BuilderDefaults.defaultIfAbsent(
        resourceListing,
  ...
  public DocumentationBuilder tags(Set<Tag> tags) {
    this.tags.addAll(BuilderDefaults.nullToEmptySet(tags).stream()
  ...
  public DocumentationBuilder produces(Set<String> mediaTypes) {
    this.produces.addAll(BuilderDefaults.nullToEmptySet(mediaTypes).stream()
        .filter(Objects::nonNull)
        .collect(toSet()));
    return this;
  }
  ...
  public DocumentationBuilder consumes(Set<String> mediaTypes) {
    this.consumes.addAll(BuilderDefaults.nullToEmptySet(mediaTypes).stream()
        .filter(Objects::nonNull)
        .collect(toSet()));
    return this;
  }
  ...
  public DocumentationBuilder schemes(Set<String> schemes) {
    this.schemes.addAll(BuilderDefaults.nullToEmptySet(schemes).stream()
        .filter(Objects::nonNull)
        .collect(toSet()));
    return this;
  }
  ...
  public DocumentationBuilder externalDocumentation(@NonNull Consumer<ExternalDocumentationBuilder> consumer) {
    ...
    consumer.accept(builder);
    this.externalDocumentation = BuilderDefaults.defaultIfAbsent(
        builder.build(),
