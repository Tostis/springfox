...
import springfox.documentation.core.service.*;
import springfox.documentation.core.service.RequestBody;
...
public class OperationBuilder {
  ...
  public OperationBuilder authorizations(Collection<SecurityReference> securityReferences) {
    List<SecurityReference> newSecurityReferences = BuilderDefaults.nullToEmptyList(securityReferences);
  ...
  public OperationBuilder tags(Set<String> tags) {
    Set<String> newTags = BuilderDefaults.nullToEmptySet(tags);
    if (!newTags.isEmpty()) {
      this.tags.clear();
  ...
  public OperationBuilder requestParameters(Collection<RequestParameter> parameters) {
    RequestParameterMerger merger = new RequestParameterMerger(
        ...
        BuilderDefaults.nullToEmptyList(parameters));
  ...
  @Deprecated
  private Set<ResponseMessage> mergeResponseMessages(
    ...
    for (ResponseMessage each : responseMessages) {
      if (responsesByCode.containsKey(each.getCode())) {
        ...
        String message = BuilderDefaults.defaultIfAbsent(ofNullable(each.getMessage())
        ...
        List<Example> examples = Stream.concat(
            ...
            ofNullable(each.getExamples()).orElse(emptyList())
                .stream())
            .collect(toList());
        ModelReference responseWithModel = BuilderDefaults.defaultIfAbsent(each.getResponseModel(),
  ...
  private Comparator<RequestParameter> defaultRequestParameterComparator() {
    return (p1, p2) -> BuilderDefaults.nullToEmpty(p1.getName()).compareToIgnoreCase(BuilderDefaults.nullToEmpty(p2.getName()));
  }
}