...
import static springfox.documentation.common.readers.operation.ResponseHeaders.*;
...
public class OpenApiResponseReader implements OperationBuilderPlugin {
  ...
  protected Set<Response> read(OperationContext context) {
    ...
    List<ApiResponse> allApiResponses
        = Stream.concat(
        operationAnnotation.map(Annotations.fromOperationAnnotation())
            .orElse(new ArrayList<>()).stream(),
        responsesAnnotation.map(Annotations.fromApiResponsesAnnotation())
            .orElse(new ArrayList<>()).stream())
  ...
  private Optional<ResolvedType> resolvedType(
      Schema apiResponse) {
    return ofNullable(Annotations.resolvedTypeFromSchema(
        typeResolver,
