...
package springfox.documentation.common.schema;
  ...
  public void apply(ModelPropertyContext context) {
    ...
    if (context.getAnnotatedElement().isPresent()) {
      annotation =
          annotation.map(Optional::of)
                    .orElse(ApiModelProperties.findApiModePropertyAnnotation(context.getAnnotatedElement().get()));
    }
    ...
    if (annotation.isPresent()) {
          ...
          annotation.map(a -> {
            if (!a.dataType().isEmpty()) {
              return modelSpecifications
                  .create(context.getOwner(), ApiModelProperties.toType(context.getResolver()).apply(a));
            }
            return null;
          })
                    .orElse(null);
      ...
      context.getSpecificationBuilder()
             .description(annotation.map(ApiModelProperties.toDescription(descriptions)).orElse(null))
             ...
             .example(annotation.map(ApiModelProperties.toExample()).orElse(null))
             .enumerationFacet(e -> e.allowedValues(finalAnnotation.map(ApiModelProperties.toAllowableValues()).orElse(null)));

      context.getBuilder()
             .allowableValues(annotation.map(ApiModelProperties.toAllowableValues()).orElse(null))
             .required(annotation.map(ApiModelProperty::required).orElse(false))
             .readOnly(annotation.map(ApiModelProperty::readOnly).orElse(false))
             .description(annotation.map(ApiModelProperties.toDescription(descriptions)).orElse(null))
             .isHidden(annotation.map(ApiModelProperty::hidden).orElse(false))
             .type(annotation.map(ApiModelProperties.toType(context.getResolver())).orElse(null))
             .position(annotation.map(ApiModelProperty::position).orElse(0))
             .example(annotation.map(ApiModelProperties.toExample()).orElse(null));
    }
  }
